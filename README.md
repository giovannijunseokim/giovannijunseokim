# 김준서 (Giovanni Junseo Kim)

> **함께 문제를 해결하고 싶은 사람**이 되기 위해 성장 중인 Android 개발자입니다.  

- 단순한 기능 구현이 아니라, 실제 사용자의 문제를 해결하는 데 집중합니다. 
- 코드를 읽는 사람이 놀라지 않도록, 일관성과 예측 가능성을 중시합니다.
- 지식과 경험을 공유하며 메타 인지를 높이고, 함께 성장하고자 합니다.

## 📱 Projects

**[런세권](https://github.com/woowacourse-teams/2025-course-pick)** | 2025 ~ | 러닝 코스 탐색 서비스

**[SOPT](https://github.com/sopt-makers/sopt-android)** | 2024 | 국내 최대 IT 벤처창업동아리 SOPT의 공식 Android 앱

**[숨쉴때](https://play.google.com/store/apps/details?id=com.yourssu.soomsil&pcampaignid=web_share)** | 2024 | 숭실대 학생들을 위한 대학라이프 종합 어플리케이션

**[낫투두](https://github.com/DO-NOTTO-DO/AOS-NOTTODO)** | 2022 ~ 2023 | 하지 말아야 할 일을 관리하는 습관 관리 서비스

## 💼 Experience

**[우아한테크코스](https://www.woowacourse.io/)** 7기 안드로이드 크루

**[YOURSSU](https://yourssu.com/)** 2024 안드로이드, 숨쉴때 V2 검색 팀

**[SOPT Makers](https://makers.sopt.org/)** 34, 35기 플랫폼 팀 안드로이드 챕터

**[SOPT](https://www.sopt.org/)** 31기 안드로이드 파트 YB, 32기 안드로이드 파트장
